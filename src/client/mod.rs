// Consumer-side Rust SDK: read-only discovery plus the spawn/topup/status DM
// round-trips, returning typed `*Outcome` enums so embedders don't hand-roll
// JSON parsing. The CLI still hand-rolls these flows in
...
impl PaygressClient {
    ...
    /// `"nip04"` (default) or `"nip17"`. NIP-17 gift-wrap is sender-anonymous
    /// but supported by fewer relays.
...
pub struct SpawnRequest {
    ...
    /// The provider's first spec is used when `None`.
...
pub struct TopupRequest {
    /// [`AccessDetailsContent::pod_npub`], e.g. `container-1234`.
    pub pod_id: String,
    pub cashu_token: String,
}

/// Result of a spawn round-trip. Anything that is neither
/// `AccessDetailsContent` nor `ErrorResponseContent` surfaces as `Other(raw)`,
/// so a provider speaking an evolved schema doesn't break the caller.
#[derive(Debug, Clone)]
...
/// `Some(err)` only when the JSON carries the discriminating `error_type` +
/// `message` fields and parses cleanly.
