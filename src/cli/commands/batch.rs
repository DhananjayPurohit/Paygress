...
fn manifest_entry_from_error(index: usize, status: &str, err: ErrorResponseContent) -> ShardManifestEntry {
    ShardManifestEntry {
        index,
...
fn manifest_entry_status_only(index: usize, status: &str, message: Option<String>) -> ShardManifestEntry {
    ShardManifestEntry {
        index,
        status: status.to_string(),
        host: None,
