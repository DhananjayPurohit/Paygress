//! Wire-format + template-resolution tests for the `template_slug` spawn
//! flow. The end-to-end path (DM → provider → docker run) runs on the VPS.
...
fn spawn_request(template_slug: Option<&str>) -> EncryptedSpawnPodRequest {
...
fn spawn_request_omits_template_slug_when_none() {
    let json = serde_json::to_string(&spawn_request(None)).unwrap();
    assert!(
        !json.contains("template_slug"),
        "a None slug must stay off the wire for old consumers"
    );
}

#[test]
fn spawn_request_includes_template_slug_when_set() {
    let json = serde_json::to_string(&spawn_request(Some("nostr-relay"))).unwrap();
...
/// template port lands on `host_port + i + 1`, never colliding with SSH.
#[test]
